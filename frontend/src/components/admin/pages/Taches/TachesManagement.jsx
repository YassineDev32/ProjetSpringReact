...
const TachesManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const carId = formData.vehicle
        ? Number.parseInt(formData.vehicle.split("-")[0].trim())
        : null;

      // Extract technician ID from the technicien string
      const technicianId = formData.technicien
        ? Number.parseInt(formData.technicien.split("-")[0].trim())
        : null;
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Créez, modifiez et assignez des tâches aux techniciens
            </p>
          </div>
          ...
          <div className="bg-gray-900 rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-gray-800">
            ...
            <form onSubmit={handleSubmit} className="p-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const selectedCar = cars.find(
                        (car) =>
                          `${car.id}` === e.target.value.split("-")[0].trim()
                      );
                      setFormData({
                        ...formData,
                  ...
                  >
                    ...
                    {cars.map((car) => (
                      <option
                        key={car.id}
                        value={`${car.id} - ${car.model?.name || "N/A"} ${
                          car.model?.mark?.name || ""
                        }`}
                      >
                        {car.model?.name || "N/A"} {car.model?.mark?.name || ""}{" "}
                        - {car.matricule}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  >
                    ...
                    {techniciens.map((tech) => (
                      <option
                        key={tech.id}
                        value={`${tech.id} - ${tech.firstname} ${tech.lastname}`}
                      >
                        {tech.firstname} {tech.lastname} -{" "}
                        {tech.role === "TECHNICIAN" ? "Technicien" : tech.role}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 text-black bg-green-500 rounded-md hover:bg-green-600"
                >
